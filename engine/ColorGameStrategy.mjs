...
export class ColorGameStrategy extends Strategy {
  static getBestMoveCandidates(game) {
    let result = super.getPossibleMoves(game, false);
    /*
    result = this.#combineTrumps(result, game);
    // over-optimization removes too much here
    result = this.#findSequenceEnds(result, game);
    this.#removeUnreasonableMoves(result, game);
    */
    return result.flat();
  }

  static #combineTrumps(cards, game) {
    if (cards[0][0]?.figure === 'J') {

      let trumpIndex = cards.findIndex(suit => suit[0]?.suit === game.gameType && suit[0].figure !== 'J');
      if (trumpIndex > -1) {
        let [trumps] = cards.splice(trumpIndex, 1);
        cards[0] = [...cards[0], ...trumps];
      }
    }
    return cards.filter(suit => suit.length > 0);
  }
